...
if EXCLUDE_SUSPENDED:
    ratings_filtered = {i: ratings[i] for i in ratings if i not in EXCLUDE_LOs and 'suspended' not in i} 
else:
    ratings_filtered = {i: ratings[i] for i in ratings if i not in EXCLUDE_LOs}
...
for i in ratings_filtered:
    ...
    if ratings_filtered[i] > UPPER_BONUS_POINT:
        print(f"Updating rating by {MULTIPLIER_SEVENTY}x")
...
ratings_filtered = {i: ratings_filtered[i] for i in ratings_filtered if (ratings_filtered[i] >= LOWER_CUTOFF_POINT and ratings_filtered[i] < UPPER_CUTOFF_POINT)}
...
for lo in ratings_processed:
    if lo in EXCLUDE_SECONDARY:
        ...
        print("[{:2}] SEC {:25}: {:10.2f}".format(ctr+2, lo, RATIO_SECONDARY*round(ratings_processed[lo], 2)))
    ctr += 2
